...
class DocReferenceHandler {
    constructor(refFieldName, sourceFields, depFields, options = {}) {
        ...
        this.options = Object.assign(this.getCommonOptions(), options);        
    }
    ...
    processTableRec(tableRec) {
        ...
        for (let i = 0; i < this.depFields.length; i++) {
            let sourceValue = catalogRecord.get(catalogRecord.getMnemo() + '.' + this.sourceFields[i]);
            if (sourceValue != null || this.options.setNullsFromSource && sourceValue == null) {
    ...
    getCommonOptions() {
        let result = {};
        switch(this.refFieldName) {
            case 'Operaciya':
                result.setNullsFromSource = false;
                break;
            default:
                result.setNullsFromSource = true;
                break;
        }
        return result;
    }
