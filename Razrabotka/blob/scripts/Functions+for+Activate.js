...
function addMonetaryАssetsPostings(currentDocument){
    ...
    function addMonetaryАssetsPosting(posting, isDb){
        ...
        if (!(sourceAccountCode in cacheAssociatedAccounts)) {
            let recAssociatedAccount = catAssociatedAccounts.findByCode(sourceAccountCode)?.[0];
        ...
        isOutlayEkr = Boolean(ekr.match(/^5(?!10).*|^[238].*/)); //200, 300, 500, 800, не 510
        ...
        if (isOutlayEkr == isDb) { //расходы по дебету или доходы по кредиту с минусом. Прямые операции с плюсом
            obj.summa *= -1;   
            swapObjProperties(obj, ['debitKps', 'creditKps']);
        }
        ...
        if (isOutlayEkr) {
            ...
            obj.debit = fillAccountByAnalytics(schet17, sourceAccount);
            obj.debitKosgu = ekr;
...
//          если в этом параметре передать не логику, а счёт, то аналитики возьмутся с него
...
//если значения нет в аналитике счета - заменяем нулевым значением
